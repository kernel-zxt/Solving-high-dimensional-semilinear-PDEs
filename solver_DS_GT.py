...
class NonsharedModel(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        y_terminal2 = y_terminal + self.bsde.delta_t * (
                self.bsde.f_tf(time_stamp[self.bsde.num_time_interval], x[:, :, -1], y_terminal, grad_T)) 
        ...
        for t in range(self.bsde.num_time_interval-2,0 ,-1):
            ...
            y = self.subnet_u[t].call(x[:, :, t ], training) + self.bsde.delta_t * (
        ...
        z = tf.multiply(self.sigma, z)
        y_t0 = self.subnet_u[0].call(x[:, :, 1 ], training) + self.bsde.delta_t * self.bsde.f_tf(time_stamp[1], x[:, :, 1], self.subnet_u[0].call(x[:, :, 1 ], training), z) 
