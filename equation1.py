# -*- coding: utf-8 -*-
"""
Created on Sun Nov 21 19:12:01 2021
...
class Equation(object):
    ...
    def sample(self, num_sample):
        ...
        #这个是调了正向微分方程的
        #raise NotImplementedError===它的意思是如果这个方法没有被子类重写，但是调用了，就会报错。
...
class HJBLQ(Equation):
    ...
    def __init__(self, eqn_config):
        ...
        self.lambd = 1.0
    #初始化参数    sigama初始化为sqrt(2);lambd=1
        

    def sample(self, num_sample):
        dw_sample = np.random.normal(size=[num_sample, self.dim, self.num_time_interval+1]) * self.sqrt_delta_t
    ...
    #keepdims：是否保持原有张量的维度，设置为True
    #lambd*(z^2+1)

    def g_tf(self, t, x):
        return tf.math.log((1 + tf.reduce_sum(tf.square(x), 1, keepdims=True)) / 2)
        #return tf.reduce_sum(x ** 2, axis=1, keepdims=True) ** 0.25


class Simplepde(Equation):
    """Simplepde equation in PNAS paper doi.org/10.1073/pnas.1718942115"""
    def __init__(self, eqn_config):
        super(Simplepde, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim)
        self.sigma = np.sqrt(2.0)
    ...
    def f_tf(self, t, x, y, z):
        return -2 * t-  6 * tf.reduce_sum((x), 1, keepdims=True)

    def g_tf(self, t, x):
        return tf.reduce_sum(tf.pow(x,3), 1, keepdims=True) - tf.reduce_sum((x), 1, keepdims=True) + 0.09 
    
    
class Simplepde1(Equation):
    """Simplepde equation in PNAS paper doi.org/10.1073/pnas.1718942115"""
    def __init__(self, eqn_config):
        super(Simplepde, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim)
        self.sigma = np.sqrt(2.0)
        self.lambd = 1/self.dim
    ...
    def f_tf(self, t, x, y, z):
        exp_term = self.dim * (lambd ** 2) * np.sin(lambd * tf.reduce_sum((x), 1, keepdims=True))
        return -2 * t + exp_term

    def g_tf(self, t, x):
        return tf.sin(lambd * tf.reduce_sum((x), 1, keepdims=True)) + 0.09


class Europeancallpde(Equation):
    """Simplepde equation in PNAS paper doi.org/10.1073/pnas.1718942115"""
    def __init__(self, eqn_config):
        super(Europeancallpde, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim) + np.log(20)
        self.sigma = 0.2
        self.mu = 0.05
    ...
    def sample(self, num_sample):
        ...
        x_sample[:, :, 0] = np.ones([num_sample, self.dim]) * self.x_init
        for i in range(self.num_time_interval):
            x_sample[:, :, i + 1] = x_sample[:, :, i]  + (mu-sigma**2/2) * delta_t  + sigma * dw_sample[:, :, i]
        return dw_sample, x_sample
    ...
    def g_tf(self, t, x):
        return np.sum(np.maximum(np.exp(x) - k,0) , 1, keepdims=True)
...
class AllenCahn(Equation):
    ...
    def g_tf(self, t, x):
        ...
        #return tf.atan(tf.reduce_max(x, axis=1, keepdims=True))
#和HJBJQ非常一致的操作
