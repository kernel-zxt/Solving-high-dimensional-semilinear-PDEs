...
def splitting_model(y, t, n, phi, f, net,  neurons, batch_size,sde,d, dtype=tf.float32):
    ...
    if net == 0:
        y1 = sde(d, n - net - 1)[:,:,1]
        v_i = phi(y1) #表示终端条件
        grad_v = tf.gradients(v_i, y1)
        v_k = v_i + t / n * f(y1, v_i, grad_v[0])
    else:
        y1 = sde(d, n - net)[:,:,1]
        v_i = neural_net(y1, neurons, 'v_%i_' % net, False, dtype=dtype)
        grad_v = tf.gradients(v_i, y1)
        v_k = v_i + t / n * f(y1, v_i, grad_v[0])
    ...
    loss = (v_j - tf.stop_gradient(v_k)) ** 2
...
def simulate(t, n, d, sde, phi, f, neurons, train_steps, batch_size,
    ...
    for i in range(n):
        ...
        with tf.Session() as sess:
            ...
            if i == n - 1:
                return sess.run(v_n),sess.run(loss)
